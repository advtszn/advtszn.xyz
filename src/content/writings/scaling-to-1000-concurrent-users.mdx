...
an EOF error is the network telling you "i opened a connection, and the other end hung up before saying anything", it means our application code never ran. something was killing connections *before* they reached Express. so the first bottleneck wasn't in our code at all, and no amount of query tuning would have touched it.
...
`blocked` over a second meant requests were queuing just to get a connection. a 7.6s TTFB meant the requests that *did* get through hit slow application logic. one was an infrastructure problem, one was a database problem, and i had to resist the urge to fix them in the wrong order.
...
two mistakes i caught before they shipped, both worth stating because they are easy to get backwards:

1. i almost deleted the `{ slug: 1, type: 1 }` unique index to "clean up." that index enforces a business rule (no duplicate slugs per content type). removing it would have silently allowed data corruption. constraint indexes and performance indexes are not interchangeable.
2. my first pass marked the new compound indexes `unique: true`. a unique index on `{ type, status, createdAt }` means two articles published in the same millisecond would fail to insert. these are query indexes, not constraints.
...
| Metric | Run 1 | Run 2 |
| --- | --- | --- |
| Success rate | 54.82% | 74.11% |
| Transport EOF | 14,751 | 7,855 |
| p95 latency | 22,221 ms | 21,758 ms |

success climbed 19 points and EOFs nearly halved, but p95 barely moved. worse, TTFB went *up*, from 7.6s to 8.5s.

that scared me until i understood it. the slowest requests in run 1 were dying as EOF before k6 could measure them. now they survived, completed, and got counted, which dragged the average up. the indexes had made individual queries faster, but the connection bottleneck was still masking it. i was measuring a problem i hadn't fixed yet.
...
| Metric | Run 3 | Run 4 |
| --- | --- | --- |
| Avg latency | 13,426 ms | 3,329 ms (-75%) |
| p95 latency | 24,794 ms | 15,293 ms (-38%) |
| Request rate | 113 req/s | 223 req/s (+96%) |

average latency dropped 75% and throughput nearly doubled. but p95 was still 15s against a 3s target, and the per-endpoint breakdown showed why: the six slowest endpoints were all writes. you can't cache a write.

# fix 5: more workers than cores (this was correct, briefly)
...
| Metric | Run 4 | Run 5 |
| --- | --- | --- |
| p95 latency | 15,293 ms | 6,857 ms (-55%) |
| Median | 468 ms | 135 ms |
| Request rate | 223 req/s | 467 req/s |

throughput doubled again. median latency fell to 135ms, so most requests were now genuinely fast, and the pain was entirely in the write-heavy tail. remember this fix. it comes back.

# fix 6 and 7: killing the write tail

two rounds of work went after the writes, which were now the entire problem.

**assembled response caching and denormalization.** i stopped caching entities and started caching final API responses (leaderboard, article detail), so a cache hit costs zero DB queries instead of one-plus-assembly. community pages were recomputing article and quiz counts on every request via `countDocuments`, so i denormalized those counts onto the community document, updated fire-and-forget on every content write, with a reconciliation job as the safety net for drift.

**write deferral, read replicas, and query parallelization.** the key insight, again from our CTO: a user who likes an article doesn't need to wait for the like *count* to increment. the frontend already does optimistic updates. so i moved engagement counters and denormalized adjustments into a BullMQ background queue, turning synchronous 3-step writes into effectively 1-step writes. i set `readPreference: 'secondaryPreferred'` so reads hit replica set secondaries and the primary is reserved for writes and auth (session and OTP lookups override back to primary, since a stale read there actually breaks things). and i wrapped every `find` + `countDocuments` pair in `Promise.all`, since they are independent and were running sequentially for no reason.

## run 7: all four thresholds green

| Write endpoint | Run 6 avg | Run 7 avg |
| --- | --- | --- |
| Like | 4,723 ms | 1,212 ms (-74%) |
| Add comment | 4,811 ms | 1,139 ms (-76%) |
| Unfollow | 4,188 ms | 1,552 ms (-63%) |

write latencies fell 56-76%. p95 landed at 2,846ms, 154ms under the 3s target, and for the first time every threshold passed. i could have stopped here. i'm glad i didn't, because the next fix taught me the most.
...
by this point the box was running, on 2 vCPUs: 4 API workers (from fix 5), 2 background job workers (both cluster mode at `instances: "max"`), plus a web and a dashboard process. that is roughly 8 Node processes fighting over 2 cores. Node is single-threaded per process, so when busy processes outnumber cores, they don't run in parallel. they context-switch, each getting yanked off the CPU mid-work, and server-side processing time balloons even though nothing is wrong with the database or the network.

p95 was stuck around 2.5s of pure server-side `waiting`, and no amount of caching would move it. so i tried something crude: mid-test, i killed the two background worker processes.
...
back at fix 5, every request sat waiting on a slow un-indexed database, so the CPU was idle during those waits and piling on extra processes was free lunch. after indexes and caching, requests are served from Redis or cheap indexed lookups. there is almost no waiting left. the time that remains is real CPU work: TLS, JSON serialization, the event loop. running 4 processes on 2 cores stopped hiding latency and started creating it. the exact same setting flipped from asset to liability.
...
so the 4-worker config was never fundamentally right. it was a workaround compensating for a slow database, and the moment i fixed the real cause, the workaround became the problem. i scaled the API back to 2 workers (one per core), moved the BullMQ processor to a single fork process (a queue consumer gets its parallelism from its `concurrency` setting, not from process count), and persisted both in `ecosystem.config.js` so they survive a deploy.

## run 8: sub-1s p95

| Metric | Run 7 (4 workers) | Run 8 (2 + 1 fork) |
| --- | --- | --- |
| p95 latency | 2,846 ms | 777 ms (-73%) |
| p99 latency (measured) | 5,676 ms | 3,714 ms |
| Throughput | 570 req/s | 756 req/s |
| Success rate | 99.93% | 100.00% |
| Requests under 1s | — | 97.3% |
| Total requests (5 min steady) | — | 307,615 |

p95 crashed to 777ms and throughput climbed to 756 req/s, both on the same 2-core box. the active CPU competitors on the latency path went from 6 cluster workers to 2, exactly matching the experiment. every one of the 307,615 requests came back successful (15 non-fatal k6 hiccups, zero real failures), and 97.3% of them landed in under a second.

one honest caveat i won't bury. that same run had a *measured* p99 of 3,714ms, which looks alarming next to a sub-1s p95. decomposing it, the entire gap is one ~15-second window where every endpoint froze in lockstep, no errors, no restart. that single stall owns the very tail of the distribution and nothing else does. exclude that one window and p99 drops to roughly 1,454ms. a box-wide freeze with no errors is the signature of a GC or swap event, and the likely cause is that every process launched with a 2GB heap ceiling, whose sum can exceed the box's RAM. right-sizing those heaps is the next thing on the list, not a per-endpoint tail to chase.
