...
description: A journey of taking a content platform's API from a 55% success rate and 22s p95 to 100% and 777ms under 1000 concurrent users.
date: 2026-07-28
---

import { Image } from "astro:assets";
import architecture from "./images/scaling-architecture.png";
import eofFd from "./images/scaling-eof-fd.png";
import cpuOversubscription from "./images/scaling-cpu-oversubscription.png";
...
the platform is a content site: articles, quizzes, polls, communities, leaderboards. and the tech stack is in the diagram below.

<Image
  src={architecture}
  alt="Request path on a single 2-core EC2 box: user to nginx, proxied to two Node.js cluster workers, reaching out to MongoDB Atlas and Redis"
  data-lightbox
/>

everything runs on a single EC2 instance with 2 CPU cores. MongoDB lives on Atlas, Redis handles leaderboard caching. i drove load with k6, simulating 1000 concurrent users across three flows: browsing (60%), quizzes (25%), and social engagement (15%).

# the EOF mystery: your app never even ran

i pointed k6 at the staging API with a load profile built to test endurance, ramp up over one minute to 1000 virtual users, then hold steady at 1000 for five minutes. this was the baseline, and it was grim.
...
more than half the requests failed, and this run had by far the most failures of the whole project. but they weren't the 500s or 503s i expected. 90.6% of them were EOFs.
...
<Image
  src={eofFd}
  alt="Under load, each client and each upstream connection eats a file descriptor. With ulimit -n at 1024, nginx caps around 500 proxied requests and request ~501 gets an EOF, all while CPU sits under 40%"
  data-lightbox
  style="max-width: 90%; max-height: 512px; height: auto; object-fit: contain;"
/>
...
`worker_rlimit_nofile` is belt-and-suspenders: even with the OS limit raised, systemd can impose its own, so nginx sets its ceiling explicitly at startup.
...
for individual content lookups i cached the domain entity data, so any use case can rehydrate a full object with `Content.rehydrate()` and not care whether it came from cache or DB. for feeds i cached the entire assembled response instead.
...
throughput doubled again to 467 req/s and p95 fell 55% to 6,857ms. median latency dropped to 135ms, so most requests were now genuinely fast, and the pain was entirely in the write-heavy tail.
...
i set `readPreference: 'secondaryPreferred'` to fan reads out across the secondaries and reserve the primary for writes. the asynchronous lag is the catch: a read from a secondary can miss a write that just happened on the primary. so i forced session and OTP lookups back to the primary, where staleness isn't acceptable.
...
<Image
  src={cpuOversubscription}
  alt="Same 2 cores, opposite optimum: while I/O-bound, ~8 processes overlap their waits and hide latency; once caching makes the work CPU-bound, 2 workers plus one fork win and p95 drops from 2846ms to 777ms"
  data-lightbox
  style="max-width: 90%; max-height: 512px; height: auto; object-fit: contain;"
/>
